...
const USER_CORRECTION = [
  ...
  /(?:i (?:already )?told you|you acknowledged.*(?:but|yet)|(?:has|have) not changed)/iu,
...
const FAILURE_ANCHOR = /(?:안\s*(?:했|됐|맞|지켰|넣|고쳤|따랐|반영)|못\s*(?:했|했어)|실패|오류|틀|누락|빠|무시|어겼|같은\s*(?:실수|질문|문제)|말만|물어|왔다\s*갔다|바뀌|되돌아|깨졌|did(?:n't| not)|failed|error|wrong|missed|ignored|same\s+(?:mistake|question|problem|thing)|(?:has|have|had)\s+not\s+changed|hasn't\s+changed|unchanged|keeps?\s+(?:asking|changing)|back\s+and\s+forth|broke|错|錯|没|沒|失败|失敗|忽略|漏|同样|同樣|没有|沒有|删|刪|擅自|同じ\s*(?:ミス|間違い|質問|問題)|できていな|直っていな|無視|見落と|戻って|変え|謝るだけ)/iu;
