...
const HEALTH_CHECK = [
  /^(?:(?:이|현재|지금|우리)\s+)?(?:(?:대화|세션)\s*상태(?:가|이|를|을)?\s*(?:(?:정상인지|드리프트(?:했는지|인지)|문제가\s*있는지)\s*)?|(?:대화|세션)(?:가|이|를|을)?\s*(?:정상인지|드리프트(?:했는지|인지)|문제가\s*있는지)\s*)(?:점검|확인|진단)(?:해\s*줘|해\s*주세요|해주세요|해|할까|해야\s*(?:해|할까|하나|하나요))\s*[?!.]*$/iu,
  /^(?:대화|세션)\s*상태\s*(?:점검|확인|진단)(?:이|가)?\s*(?:필요해|필요한가|필요할까|필요하냐|필요한지)\s*[?!.]*$/iu,
  /^(?:새\s*세션|컴팩션)(?:이|을|를|으로)?\s*(?:필요해|필요한가|필요할까|해야\s*(?:해|할까|하나|하나요)|시작(?:할까|해야\s*(?:해|할까|하나|하나요))|사용할까)\s*[?!.]*$/iu,
  ...
  /^(?:(?:我们|我們)\s*)?(?:需要|要不要)(?:开始|開始|开启|開啟|换到|換到|使用)?(?:一个|一個)?(?:新对话|新對話|新会话|新會話)(?:吗|嗎)?\s*[？?。.!]*$/u,
  /^(?:(?:この|現在の|今の)\s*)?(?:(?:会話|セッション)(?:の?状態)(?:が|は|を)?\s*(?:(?:ずれているか|おかしいか|健全か|正常か|ドリフトして(?:いる|いない)か)\s*)?|(?:会話|セッション)(?:が|は|を)?\s*(?:ずれているか|おかしいか|健全か|正常か|ドリフトして(?:いる|いない)か)\s*)(?:確認|診断|点検)(?:して|してください|してほしい|しますか|しましょうか)\s*[?？.!。]*$/u,
  /^(?:(?:この|現在の|今の)\s*)?(?:会話|セッション)(?:が|は)?\s*(?:ずれていますか|ずれていませんか|おかしいですか|ドリフトしていますか|ドリフトしていませんか)[?？](?:\s*状態を確認しますか[?？])?$/u,
  /^(?:新しいセッション(?:が|を|に)?\s*(?:必要(?:ですか)?|始め(?:ますか|るべき)|移る(?:方が(?:いい|よい)(?:ですか)?|べき(?:ですか)?)|使う(?:方が|べき))|コンパクションした方が(?:いい|よい|いいですか|よいですか)?)\s*[?？.!。]*$/u,
];
